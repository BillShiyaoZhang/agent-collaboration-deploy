...
EXPECTED = {"agent-comm-runtime": "0.1.0", "hermes-platform-agent-comm": "1.3.0"}
...
def verify_bundle(directory):
    ...
    for wheel in wheels:
        ...
        if name not in EXPECTED or metadata.get("Version") != EXPECTED[name] or name in selected:
...
def main(argv=None):
    ...
    try:
        ...
        print(json.dumps({"status": "bundle_verified", "bundle": str(root), "packages": EXPECTED}, ensure_ascii=False))
        if args.check_only:
            return 0
        ...
        command = [sys.executable, "-m", "pip", "--isolated", "install", "--disable-pip-version-check", "--no-input",
                   ...
                   "--force-reinstall", *(str(wheels[name]) for name in EXPECTED)]
        ...
        probe = ("import importlib.metadata as m; "
                 "assert m.version('agent-comm-runtime') == '0.1.0'; "
                 "assert m.version('hermes-platform-agent-comm') == '1.3.0'; "
