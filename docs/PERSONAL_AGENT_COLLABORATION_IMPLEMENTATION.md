...
更新：2026-09-14。本轮依据用户明确的产品边界完成重构并部署早期试用版。当前设计与源码关系见 [架构图](PROJECT_ARCHITECTURE_DIAGRAMS.md) 和 [扩展接口](ARCHITECTURE_AND_EXTENSION_PORTS.md)，公网及回滚记录见 [发布记录](EARLY_ACCESS_RELEASE_2026-09-14.md)。此前 1.2.0 的“所有内核都在 Hermes 包内”结构已被下述实现取代。

## 当前产品边界

Web 是连接用户 Agent 的远程工作台，保留登录账户、控制台身份、每用户连接记录及有限期 RPC 密文缓存。联系人、任务、审批、收件、对话结果均由 agent 侧返回。旧 Web domain 模型和模拟页面/API 不再由应用维护；迁移保留旧表和数据，不在启动时清库。
...
远程配对撤销与响应提交按本地事务顺序处理；撤销阻止后续 RPC、agent 侧尚未提交给 helper 的缓存响应和未执行会话。已经返回 Web 的短期缓存及已披露内容无法召回，已经运行的宿主工具不能回滚。Web 的 10 分钟缓存期限是可读取有效期，过期记录在后续控制调用或轮询时清理。Web 后端是托管控制台身份端点，会解密用于显示的内容；云 MQ 保存签名密文及路由元数据。
