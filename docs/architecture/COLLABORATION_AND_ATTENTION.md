...
本文描述双边协作与提醒的实现边界。安装与部署步骤见[运维入口](../operations/README.md)，特定版本的实机结果见[验收记录](../verification/README.md)。尚未实现的扩展见[后续方向](../developers/FUTURE_DIRECTIONS.md)。

## 实现了什么

| 部分 | 当前能力 |
| --- | --- |
| 本地 runtime | v2 双边协议、独立任务绑定、同版接受与约定、撤回/取消/恢复；持久 attention、本人详情、处理会话账本及有限确定性 worker |
| Hermes connector | 真实原生确认及到期清卡、profile 身份隔离、后台定时推进、事项详情与处理会话 API、符合实际工具参数的交接提示 |
| Hermes companion | 待办详情与当前版本刷新、恢复或新建持久处理会话、唯一会话标题、单次处理请求、未读/待处理计数、系统通知与诊断 |
...
图中绿色强调自动后台步骤，黄色标记需要本人介入、独立证据不足或由外部环境决定的边界。
...
sequenceDiagram
    ...
    alt 已绑定或原始会话仍存在
        ...
        H->>N: 创建处理会话
        H->>N: 持久化带真实会话 ID 后缀的唯一标题
        H->>R: 持久绑定，采用并发胜出的会话
    end
    H->>N: 打开并载入绑定会话，核对会话 ID
    H->>R: claim-submit，单次认领处理请求
    alt 首次认领成功
        H->>N: prompt.submit，提交核对上下文、展示问题请求
        H->>R: 记录 submitted 或 uncertain
    else 已提交或结果不确定
        Note over H,N: 保持已打开的绑定会话，不自动重发
    end
...
Web 从认证响应生成加密副本，并原子保存事项和游标。闭页推送使用加密持久化的 VAPID 与设备订阅，载荷只有随机投递 ID、订阅绑定及期限；worker 展示前通过当前登录会话重新核验，标题和正文始终为固定概括。页面可见但失去焦点属于离开状态，有焦点时普通系统提醒延后。页面和 Push 共用设备认领；重复投递补回已存回执，关闭提醒或切换账号会串行完成在途显示清理。
...
部署条件、厂商允许列表、诊断与回滚方式见 [Web Push 运维说明](../../agent-collaboration-web/docs/operations/WEB_PUSH.md)。源码与隔离测试不能代替实机验收；已做过的验证见[验收记录](../verification/README.md)。

旧配对不会自动扩大为 `attention.list` 或 `approval.respond`。用户需在本机明确更新方法配对，Web 才能使用完整提醒 feed 或对具体请求显示远程批准按钮；已有只读状态同步可提供保守的待确认/来信显示。批准仍需本人在对应事项中明确操作，聊天文字不能代答。
