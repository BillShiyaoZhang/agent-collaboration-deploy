...
1. A、B 各自在本机生成和保存 Ed25519 身份私钥。URN 的指纹可证明所呈现的 Ed25519 公钥属于该 URN；握手签名进一步证明对端持有相应私钥。若主人要认定“这个 URN 是现实中的 B 所使用的 agent”，须从 Platform 之外的可信渠道核对**准确 URN**。初次仅从同一个 Platform 的搜索结果或同一个 Web 授权页获得 URN，不能抵御平台把错误 URN 说成 B。好友申请可由真实的某个 URN 发起，仍不能据此确认其现实身份。接受申请只建立通讯关系，不提高 `trusted`、授权任务或许可披露。已有手工固定的完整公钥继续有效；若它与 Registry 记录不一致，拒绝静默替换。更换身份密钥意味着新的 URN，须重新确认。
...
3. Registry 记录由 B 的身份密钥签署其长期 X25519 公钥。更新后的 v2 helper 解析准确 URN 时，验证 URN 指纹、Ed25519 公钥、派生的 PeerID、X25519 公钥与记录签名，再缓存已验证的绑定；已有手工 pin 须与查询结果一致，不能被自动结果覆盖。v2 的 `recipient_key_id` 由 X25519 公钥摘要得出。Registry 的密码学校验可防止平台在同一 URN 下替换成另一把身份公钥，但不能说明谁在现实中持有该身份。当前不覆盖跨 Platform 查找、路由或互信。
