...
普通聊天输入框的“可以”可能已开启新回合，不能批准旧问题。新版源码的 Web 支持通过单独获准的 `approval.respond` 回答具体审批：Agent 从已验证控制台的本地配对导出主人主体，检查审批归属、期限、事项撤销与内容版本，并写入现有审批状态。Web 的明确决定可结束该审批的旧原生租约，迟到的本机回答不能覆盖结果。远程对话权限不包含此确认权；宿主本身若另外授予任意文件和工具访问，仍需由宿主约束。

`contacts.add` 允许用户在 Web 表单中确认联系人 ID、别名和 URN 的绑定，直接落到同一个 Agent Store，无需模型代填或执行。两项写操作仅由用户提交触发；结果仍通过 agent 的读取接口同步到 Web 副本。`approval.respond` 只改变审批/授权记录，后续执行仍经过正常策略检查。现有配对需在本机显式更新；需要发布匹配的 Agent/runtime 和 Web 后才能使用。
...
| Web 添加联系人、回答具体审批 | 新版源码支持 `contacts.add` / `approval.respond`；需匹配发布和显式配对授权，结果保存在 agent |
