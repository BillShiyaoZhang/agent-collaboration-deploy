...
[可验证的隐私与合规解密 v2](COMPLIANCE_GATEWAY.md)：初版源码、信任边界、本地验收与尚未实现的加强项；现网签名策略为 `compliance`（epoch 3 长期策略、`allow_v1=false`），普通旧 v1 Agent 间路径被拒，Relay 禁用。2026-09-24 发布的 v0.8.0 接入包仍要求手工固定对端完整公钥；v0.9.0 支持同一 Platform 下凭准确 URN 首联。当前公开版本以实际发布清单为准。
