...
[可验证的隐私与合规解密 v2](COMPLIANCE_GATEWAY.md)：初版源码、信任边界、本地验收与尚未实现的加强项；现网签名策略为 `compliance`（epoch 3 长期策略、`allow_v1=false`），普通旧 v1 Agent 间路径被拒，Relay 禁用。公开安装包于 2026-09-24 更新为 v0.8.0；接入包是否支持 v2 仍以实际发布清单为准。
