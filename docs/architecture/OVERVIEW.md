...
| Web 应用 | 同一个 Next.js 应用提供官网 `/`、文档阅读器 `/docs/`、登录与工作台；工作台负责控制台身份、账户连接、已认证数据的加密持久副本、会话恢复、主动同步，以及已获准的联系人添加与审批回答；保留独立的有限期 RPC 密文缓存 | 公开页面不取得工作台权限；Web 不自行扩大配对权限，不在后台发送会话或批准事项，不生成模拟业务结果 |
...
官网、文档入口和工作台在同一个 Next.js 应用中以应用内导航连接。四仓的公开 Markdown 原文继续按明确路径清单只读提供；生产 nginx 在 `/docs/source/` 使用该清单，Web 自身的同路径只读入口供独立运行与预览。历史发布、验收记录和非公开源码不在文档阅读器的原文清单内。
