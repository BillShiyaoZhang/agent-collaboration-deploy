...
核对日期：2026-09-15。本文依据部署配置和锁定版本源码，解释当前实现。图中的处理步骤适度合并，状态名称、数据归属和关键提交顺序对应代码。
...
本次部署工作树的子模块目录为空，核对使用同一电脑已有的干净源码检出；其 HEAD 与本仓库 gitlink 完全一致。没有把最新分支或历史设计稿当成锁定版本实现。

| 仓库 | 本次核对提交 |
| --- | --- |
| agent-collaboration-deploy | `e386928`（本文添加前） |
...
依据：[Compose](../docker-compose.yml)、[nginx](../nginx.conf)、[平台配置](../config.yaml)、[helper 启动及本机访问检查][s-daemon]。
...
源码：[Web 控制服务][w-control]、[提交台账][w-store]、[RemoteBridge][s-remote]、[9/15 验收](HERMES_COOPERATION_TEST_2026-09-15.md)。
...
依据：[安装器](../tools/early_access/install.py)、[配置器](../tools/early_access/configure_hermes.py)、[打包器](../tools/build_early_access.py)、[同步发布](WORKSPACE_SYNC_RELEASE_2026-09-14.md)、[9/15 验收](HERMES_COOPERATION_TEST_2026-09-15.md)。
...
最近的 [2026-09-15 验收记录](HERMES_COOPERATION_TEST_2026-09-15.md) 记载 runtime 40、Hermes Connector 113、Web 114，共 267 项通过；另有 12 项真实 helper/platform 网络检查，以及上线后真实 Hermes 回显、重复提交和 workspace 自动保存结果验证。这些是已有验收记录，**本次是源码梳理，没有重新运行测试或访问生产确认在线状态**。
