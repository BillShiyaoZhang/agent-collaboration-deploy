...
更新：2026-09-14。用户要求登录后由服务端保存连接、联系人和消息，并主动同步。本轮据此增加账户范围的持久工作台副本和后台同步；agent 侧继续提供业务事实、执行与配对授权，并适配不同宿主、记忆系统与用户交互方式。本文描述代码方向，验证和上线状态见发布记录。
...
flowchart TB
    ...
    WEBSTORE[("账户隔离的加密工作台副本<br/>联系人、消息、已知会话与同步状态")]
    WORKER["常驻 Node 同步 worker<br/>权限检查、只读 RPC、租约与退避"]
    CLOUD["云 Registry / MQ"]
    NATIVE <--> HOST
    NATIVE <--> INTERACTION
    WEB <-->|"认证加密 RPC"| CLOUD
