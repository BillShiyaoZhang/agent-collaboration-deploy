...
基于后续评审，本轮实现采用以下约束。下文 Persona 与 storyboard 的需求推导继续适用；第 8 节保留初版导航设想，当前界面以本节为准，实际上线证据由当次发布记录给出。

- 电脑浏览器为默认使用环境。进入“我的 agents”即显示聊天框，多个 agent 也直接选择一个现有连接；不先展示大卡片或要求再打开工作台。
- 桌面采用“导航｜agent 与聊天列表｜聊天框”。导航、列表和消息各有明确区域；消息独立滚动，输入框保留在底部。手机显示聊天与可收起的列表。
- 合作、联系人、我、连接设置各自有独立页面。旧混合标签页只兼容跳转到对应页面，保留来源对话与具体事项参数。
- agent 可改名和删除连接；聊天可改名、归档、删除和恢复；联系人与已确认结束的合作可删除 Web 列表记录并恢复。正在执行、审批未处理或写入结果未知时保留核实入口并阻止不适用的删除。
- 删除连接会删除此账户下的 Web 副本；联系人和合作的列表删除保留 agent 本机事实。两种删除都不表示停止远端执行或撤销本机配对，确认框说明实际范围。

```mermaid
flowchart LR
    subgraph Desktop["电脑默认聊天界面"]
        direction LR
        Nav["导航<br/>我的 agents / 合作 / 联系人 / 我"]
        List["agent 列表与聊天主题<br/>选择、搜索、管理、恢复"]
        Chat["当前 agent 与主题<br/>独立滚动的消息<br/>底部输入框"]
        Nav --> List --> Chat
    end
    Nav --> Coop["独立合作页面<br/>判断进展、发起、决定、管理"]
    Nav --> Contacts["独立联系人页面<br/>好友申请、通信、管理"]
    Nav --> Me["账户与授权<br/>连接设置、提醒设置"]
```
